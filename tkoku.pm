	...
	COUNT => {			# FUNCTION PARAMETER	$funcp
		...
		graphp_mode => {												# New version of graph pamaeter for each MODE
			NC => [
				...
				{ext => "#KIND# Tokyo TOP01-60 1m (#LD#) #SRC#", start_day => -31,  lank =>[0, 39] , exclusion => $EXC, target => "", label_skip => 1, graph => "lines", avr_date => 7, thresh => 1, term_ysize => 600},
				{ext => "#KIND# Tokyo TOP01-20 1m (#LD#) #SRC#", start_day => -31,  lank =>[0, 19] , exclusion => $EXC, target => "", label_skip => 1, graph => "lines", avr_date => 7, thresh => 1, term_ysize => 450},
				{ext => "#KIND# Tokyo TOP021-40 1m (#LD#) #SRC#", start_day => -31,  lank =>[20, 39] , exclusion => $EXC, target => "", label_skip => 1, graph => "lines", avr_date => 7, thresh => 1, term_ysize => 450},
				{ext => "#KIND# Tokyo TOP041-60 1m (#LD#) #SRC#", start_day => -31,  lank =>[40, 59] , exclusion => $EXC, target => "", label_skip => 1, graph => "lines", avr_date => 7, thresh => 1, term_ysize => 450},
