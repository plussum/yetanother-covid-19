	...
	COUNT => {			# FUNCTION PARAMETER	$funcp
		...
		graphp_mode => {												# New version of graph pamaeter for each MODE
			NC => [
				...
				{ext => "#KIND# Tokyo 1-5 $STD (#LD#) #SRC# ", start_day => "$STD",  lank =>[0, 4] , exclusion => "", target => "", label_skip => 7, graph => "lines", avr_date => 7},
				...
				{ext => "#KIND# Tokyo 1-5 $STD (#LD#) #SRC#", start_day => "$STD",  lank =>[0, 4] , exclusion => "", target => "", label_skip => 7, graph => "lines"},
