...
#
my $IM_CSVF = "$WIN_PATH/ft-im-$MODE.csv.txt";
my $FT_CSVF = "$WIN_PATH/cov_ft_$MODE" . ".csv.txt";
...
my $EXCLUSION = "Others";
my @PARAMS = (
	{ext => "$mode Japan and others FT $TD", start_day => 0, lank =>[0, 999] , exclusion => $EXCLUSION, add_target => "Japan",
...
my @csvlist = (
    { name => "$mode", csvf => $FT_CSVF, htmlf => $GRAPH_HTML, kind => "NC"},
