...
my $pop_target = "New Jersey,New York,Louisiana,Arizona,Florida,Mississippi,Alabama,Georgiai,Texas,CaliforniaA";
my $CCSE_BASE_DIR = "/home/masataka/who/COVID-19/csse_covid_19_data/csse_covid_19_time_series";
my @usa_params = (
	{ext => "#KIND# TOP 10 (#LD#) #SRC#", start_day => 0, lank =>[0, 9], exclusion => $EXCLUSION, target => "", label_skip => 7, graph => "lines", avr_date => 7},
	{ext => "#KIND# TOP 10 (#LD#) #SRC#", start_day => 0, lank =>[0, 9], exclusion => $EXCLUSION, target => "", label_skip => 7, graph => "lines"},
	{ext => "#KIND# TOP 10 targeted (#LD#) #SRC#", start_day => 0, lank =>[0, 9], exclusion => $EXCLUSION, target => $pop_target, label_skip => 7, graph => "lines"},
...
#	{ext => "#KIND# TOP 10 05/01 Arizona  (#LD#) #SRC#", start_day => "2020/05/01", lank =>[0, 9], exclusion => $EXCLUSION, target => "Arizona", label_skip => 3, graph => "lines"},
#	{ext => "#KIND# TOP 10 05/01 Florida  (#LD#) #SRC#", start_day => "2020/05/01", lank =>[0, 9], exclusion => $EXCLUSION, target => "Florida", label_skip => 3, graph => "lines"},
#	{ext => "#KIND# TOP 10 05/01 Oklahoma (#LD#) #SRC#", start_day => "2020/05/01", lank =>[0, 9], exclusion => $EXCLUSION, target => "Oklahoma", label_skip => 3, graph => "lines"},
##	{ext => "#KIND# TOP 10 05/01 Oregon (#LD#) #SRC#", start_day => "2020/05/01", lank =>[0, 9], exclusion => $EXCLUSION, target => "Oregon", label_skip => 3, graph => "lines"},
#	{ext => "#KIND# TOP 10 05/01 Texas (#LD#) #SRC#", start_day => "2020/05/01", lank =>[0, 9], exclusion => $EXCLUSION, target => "Texas", label_skip => 3, graph => "lines"},
...
#	},
	...
	COUNT => {			# FUNCTION PARAMETER    $funcp
		...
		graphp => [		# GPL PARAMETER         $gplp					# Old version of graph parameter
			{ext => "#KIND# TOP 10 (#LD#) #SRC#", start_day => 0, lank =>[0, 19], exclusion => $EXCLUSION, target => "", label_skip => 7, graph => "lines"},
		],
	...
	ERN => {
		...
		average_date => 7,
		graphp => [
			{ext => "#KIND# US 01/23 #RT_TD#", start_day => 0, lank =>[0, 9] , exclusion => "", target => "", label_skip => 7, graph => "lines"},
			{ext => "#KIND# US 2month #RT_TD#", start_day => -62, lank =>[0, 9] , exclusion => "", target => "", label_skip => 3, graph => "lines"},
	...
	KV => {
		EXC => "",
		graphp => [
			{ext => "#KIND# USA (#LD#) #SRC#", start_day => "2020/03/15",  lank =>[0, 9], exclusion => "", target => "", label_skip => 7, graph => "lines"},
			{ext => "#KIND# USA 11-20(#LD#) #SRC#", start_day => "2020/03/15",  lank =>[10, 19], exclusion => "", target => "", label_skip => 7, graph => "lines"},
