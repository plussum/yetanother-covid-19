...
#	{ext => "EOD"},
	...
	# GPL PARAMETER			$gplp
	{ext => "#KIND# Japan TOP20 (#LD#) #SRC#", start_day => "2020/03/01",  lank =>[0, 19] , exclusion => $EXCLUSION, target => "", label_skip => 7, graph => "lines"},
	{ext => "#KIND# Japan 01-05 (#LD#) #SRC#", start_day => "2020/03/01",  lank =>[0, 4] , exclusion => $EXCLUSION, target => "", label_skip => 7, graph => "lines"},


	{ext => "#KIND# Japan 01-08 (#LD#) #SRC# rl-avr", start_day => "2020/03/01",  lank =>[0, 7] , exclusion => $EXCLUSION,
		 target => "", label_skip => 7, graph => "lines", avr_date => 7, additional_plot => 0.5},
#	{ext => "#KIND# Japan 01-08 (#LD#) #SRC# rl-avr ymax", start_day => "2020/03/01",  lank =>[0, 7] , exclusion => $EXCLUSION,
...
#		 target => "", label_skip => 1, graph => "lines", additional_plot => 0.5},
	...
	{ext => "#KIND# Japan 02-05 (#LD#) #SRC#", start_day => "2020/03/01",  lank =>[1, 4] , exclusion => $EXCLUSION, target => "", label_skip => 7, graph => "lines"},
	{ext => "#KIND# Japan 06-10 (#LD#) #SRC#", start_day => "2020/03/01",  lank =>[5, 9] , exclusion => $EXCLUSION, target => "", label_skip => 7, graph => "lines"},
	{ext => "#KIND# Japan 11-15 (#LD#) #SRC#", start_day => "2020/03/01",  lank =>[10, 14] , exclusion => $EXCLUSION, target => "", label_skip => 7, graph => "lines"},
	{ext => "#KIND# Japan 16-20 (#LD#) #SRC#", start_day => "2020/03/01",  lank =>[15, 20] , exclusion => $EXCLUSION, target => "", label_skip => 7, graph => "lines"},
	{ext => "#KIND# Japan 01-10 log (#LD#) #SRC#", start_day => "2020/03/01",  lank =>[0, 9] , exclusion => $EXCLUSION, target => "", label_skip => 7, graph => "lines",
		 logscale => "y", average_date => 7},

	{ext => "#KIND# taget cities  (#LD#) #SRC# 02/01", start_day => "2020/02/01",  lank =>[0, 99] , exclusion => $EXCLUSION, 
		target => "東京,大阪,神戸,北海道,神奈川,埼玉,千葉,北海道", label_skip => 7, graph => "lines"},
	{ext => "#KIND# taget cities  (#LD#) #SRC# 03/01 ", start_day => "2020/03/01",  lank =>[0, 99] , exclusion => $EXCLUSION, 
		target => "東京,大阪,神戸,北海道,神奈川,埼玉,千葉,北海道", label_skip => 7, graph => "lines", term_ysize => 300},
	{ext => "#KIND# taget cities  (#LD#) #SRC# 03/01 rl-avr", start_day => "2020/03/01",  lank =>[0, 99] , exclusion => $EXCLUSION, 
		target => "東京,大阪,神戸,北海道,神奈川,埼玉,千葉,北海道", label_skip => 7, graph => "lines", avr_date => 7, term_ysize => 300},

#	{ext => "#KIND# Fukuoka,Hokaido (#LD#) #SRC# 03/01", start_day => "2020/03/01",  lank =>[0, 99] , exclusion => $EXCLUSION, 
#		target => "福岡,北海道", label_skip => 7, graph => "lines", term_ysize => 300},
#	{ext => "#KIND# Fukuoka,Hokaido (#LD#) #SRC# 03/01 rl-avr", start_day => "2020/03/01",  lank =>[0, 99] , exclusion => $EXCLUSION, 
...
our $PARAMS = {			# MODULE PARETER		$mep
	...
	COUNT => {			# FUNCTION PARAMETER	$funcp
		...
		graphp_mode => {												# New version of graph pamaeter for each MODE
			...
			CC => [
				{ext => "#KIND# Japan TOP20 (#LD#) #SRC#", start_day => "2020/02/15",  lank =>[0, 19] , exclusion => "", target => "", label_skip => 7, graph => "lines"},
				{ext => "#KIND# Japan 01-10 (#LD#) #SRC#(wo 東京)", start_day => "2020/02/15",  lank =>[0, 9] , exclusion => "東京", target => "", label_skip => 7, graph => "lines"},
				{ext => "#KIND# Japan 11-19 (#LD#) #SRC#", start_day => "2020/02/15",  lank =>[10, 19] , exclusion => $EXCLUSION, target => "", label_skip => 7, graph => "lines"},
				{ext => "#KIND# Japan 21-29 (#LD#) #SRC#", start_day => "2020/02/15",  lank =>[20, 29] , exclusion => $EXCLUSION, target => "", label_skip => 7, graph => "lines"},
				{ext => "#KIND# Japan 31-39 (#LD#) #SRC#", start_day => "2020/02/15",  lank =>[30, 39] , exclusion => $EXCLUSION, target => "", label_skip => 7, graph => "lines"},
			],
		},
	},
	FT => {
	...
	ERN => {
		...
		average_date => 7,
		graphp => [	
			{ext => "#KIND# Japan 0301 #RT_TD#", start_day => "2020/02/01", lank =>[0, 5] , exclusion => $EXCLUSION, taget => "",
			...
			{ext => "#KIND# Tokyo 0301 #RT_TD#", start_day => "2020/02/01", lank =>[0, 5] , exclusion => $EXCLUSION, target => "東京,大阪,神戸,北海道", 
				label_skip => 7, graph => "lines", term_ysize => 300, ymax => 10},
	...
	KV => {
		...
		graphp => [
			...
			{ext => "#KIND# from 03/01 Tokyo (#LD#) #SRC#", start_day => "2020/03/01",  lank =>[0, 999], exclusion => $EXCLUSION, 
...
#			{ext => "#KIND# from 03/01 log(#LD#) #SRC#", start_day => "2020/03/01",  lank =>[0, 999], exclusion => $EXCLUSION, 
