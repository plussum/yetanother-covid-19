...
#		 target => "", label_skip => 1, graph => "lines", additional_plot => 0.5},
	...
	{ext => "#KIND# taget cities  (#LD#) #SRC# 03/01 ", start_day => "03/01",  lank =>[0, 99] , exclusion => $EXCLUSION, 
		target => "東京,大阪,神戸,北海道,神奈川,埼玉,千葉,北海道", label_skip => 2, graph => "lines", term_ysize => 300},
...
our $PARAMS = {			# MODULE PARETER		$mep
	...
	COUNT => {			# FUNCTION PARAMETER	$funcp
		...
		graphp_mode => {												# New version of graph pamaeter for each MODE
			...
			CC => [
				{ext => "#KIND# Japan TOP20 (#LD#) #SRC#", start_day => "02/15",  lank =>[0, 19] , exclusion => "", target => "", label_skip => 2, graph => "lines"},
				{ext => "#KIND# Japan 01-10 (#LD#) #SRC#(wo 東京)", start_day => "02/15",  lank =>[0, 9] , exclusion => "東京", target => "", label_skip => 2, graph => "lines"},
				{ext => "#KIND# Japan 11-19 (#LD#) #SRC#", start_day => "02/15",  lank =>[10, 19] , exclusion => $EXCLUSION, target => "", label_skip => 2, graph => "lines"},
				{ext => "#KIND# Japan 21-29 (#LD#) #SRC#", start_day => "02/15",  lank =>[20, 29] , exclusion => $EXCLUSION, target => "", label_skip => 2, graph => "lines"},
				{ext => "#KIND# Japan 31-39 (#LD#) #SRC#", start_day => "02/15",  lank =>[30, 39] , exclusion => $EXCLUSION, target => "", label_skip => 2, graph => "lines"},
			],
		},
	},
	FT => {
