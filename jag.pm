...
our $PARAMS = {			# MODULE PARETER		$mep
	...
	COUNT => {			# FUNCTION PARAMETER	$funcp
		...
		graphp_mode => {												# New version of graph pamaeter for each MODE
			...
			CC => [
				{ext => "#KIND# Japan TOP20 (#LD#) #SRC#", start_day => "02/15",  lank =>[0, 19] , exclusion => $EXCLUSION, target => "", 
					label_skip => 2, graph => "lines"},
				{ext => "#KIND# Japan 01-05 (#LD#) #SRC#", start_day => "02/15",  lank =>[0, 4] , exclusion => $EXCLUSION, target => "", 
					label_skip => 2, graph => "lines"},
			],
		},
	},
