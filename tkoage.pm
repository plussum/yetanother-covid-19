	...
	COUNT => {			# FUNCTION PARAMETER	$funcp
		...
		graphp_mode => {												# New version of graph pamaeter for each MODE
			NC => [
				...
				{ext => "#KIND# Tokyo Age (#LD#) #SRC# ", start_day => 0,  lank =>[0, 19] , exclusion => $EXC, target => "", label_skip => 7, graph => "lines", 
						avr_date => 7, nosort => 1},
				{ext => "#KIND# Tokyo Age (#LD#) #SRC# ruiseki +1", start_day => 0,  lank =>[0, 19] , exclusion => $EXC, target => "", label_skip => 7, graph => "lines", 
				...
				{ext => "#KIND# Tokyo over 50 (#LD#) #SRC# ", start_day => 0,  lank =>[0, 19] , exclusion => $EXC, 
